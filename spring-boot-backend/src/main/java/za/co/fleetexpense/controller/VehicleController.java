...
public class VehicleController {
    ...
    public ResponseEntity<VehicleDTO> recalculateOdometer(@PathVariable UUID id) {
        return ResponseEntity.ok(vehicleService.recalculateOperationalOdometer(id));
    }

    @PostMapping("/{id}/correct-operational-baseline")
    public ResponseEntity<VehicleDTO> correctOperationalBaseline(
            @PathVariable UUID id,
            @RequestParam Integer newBaseline,
            @RequestParam String justification,
            @AuthenticationPrincipal UserPrincipal principal) {
        // Allow both authorized fleet roles and individual vehicle owners
        // The service method will handle authorization checks
        return ResponseEntity.ok(vehicleService.correctOperationalBaseline(id, newBaseline, justification, principal.getUserId()));
    }
