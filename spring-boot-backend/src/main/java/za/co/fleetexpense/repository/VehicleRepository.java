...
public interface VehicleRepository extends JpaRepository<Vehicle, UUID> {
    ...
    /**
     * Phase 1: Pessimistic lock for concurrency safety during operational odometer recalculation.
     * Locks the vehicle row with FOR UPDATE to prevent concurrent modifications.
     */
    @Query("SELECT v FROM Vehicle v WHERE v.id = :id")
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    java.util.Optional<Vehicle> findByIdWithLock(@Param("id") UUID id);
}
