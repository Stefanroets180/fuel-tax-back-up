...
public class MechanicServiceService {
    ...
    public MechanicService create(MechanicService mechanicService) {
        ...
        // Phase 1: Child-only mutation hook - recalculate after child serviceType is persisted
        // This is a separate code path from ExpenseService generic hooks (which fire when parent Expense is mutated)
        // No duplication: ExpenseService hooks handle parent mutations, this handles child-only mutations
        if (saved.getExpense().getVehicle() != null) {
            vehicleService.recalculateOperationalOdometer(saved.getExpense().getVehicle().getId());
        }
    ...
    public MechanicService update(UUID id, MechanicService updatedService) {
        ...
        ServiceType oldServiceType = existing.getServiceType();
        ...
        // Phase 1: Child-only mutation hook - recalculate if serviceType changed
        // This is a separate code path from ExpenseService generic hooks
        // No duplication: only triggers on child serviceType changes
        if (!java.util.Objects.equals(oldServiceType, saved.getServiceType()) && saved.getExpense().getVehicle() != null) {
            vehicleService.recalculateOperationalOdometer(saved.getExpense().getVehicle().getId());
        }
    ...
    public void delete(UUID id) {
        ...
        UUID vehicleId = service.getExpense().getVehicle() != null ? service.getExpense().getVehicle().getId() : null;

        mechanicServiceRepository.delete(service);
        log.info("Deleted mechanic service {}", id);

        // Phase 1: Child-only mutation hook - recalculate after child deletion
        // This is a separate code path from ExpenseService generic hooks
        // No duplication: only triggers on child deletion
        if (vehicleId != null) {
