...
public class TripService {
    ...
    /**
     * Get last odometer reading for trip autofill.
     * Phase 2: Corrected to use trip history only, not expense max.
     * 
     * Semantics:
     * - If previous trip history exists: prefill from previous Trip Log endOdometer
     * - Else: fallback to Vehicle operational current odometer (stored canonical value)
     * 
     * Does NOT use qualifying Expense max directly in Trip Log autofill.
     * The operational current already represents the operational domain.
     * 
     * Trip ordering: createdAt DESC, id DESC (most recently ENTERED trip)
    ...
    public Map<String, Object> getLastOdometerReading(UUID vehicleId, UUID userId, UserRole role) {
        try {
            ...
            log.info("Vehicle found: {}, stored operational current odometer: {}", vehicle.getRegistrationNumber(), vehicle.getCurrentOdometerStored());

            // Phase 2: Use trip history only for autofill
            // Spring Data findFirst guarantees at most one result
            Integer lastTripEndOdometer = tripRepository
                    .findFirstByVehicleIdOrderByCreatedAtDescIdDesc(vehicleId)
                    .map(Trip::getEndOdometer)
                    .orElse(null);
            
            // Fallback to stored canonical operational current odometer if no trip history exists
            // Use getCurrentOdometerStored() to avoid transient computed value
            Integer lastOdometer;
            if (lastTripEndOdometer != null) {
                lastOdometer = lastTripEndOdometer;
            } else {
                lastOdometer = vehicle.getCurrentOdometerStored();
            }

            log.info("Trip autofill: lastTripEndOdometer={}, storedOperationalCurrentOdometer={}, autofillValue={}", 
                    lastTripEndOdometer, vehicle.getCurrentOdometerStored(), lastOdometer);

            Map<String, Object> response = new HashMap<>();
            response.put("vehicleId", vehicleId);
            response.put("lastOdometer", lastOdometer);
            response.put("currentOdometer", vehicle.getCurrentOdometerStored());
    ...
    public TripDTO create(TripCreateRequest request, UUID organizationId, UUID userId, UserRole role) {
        ...
        if (startOdometer != null && request.getEndOdometer() != null) {
            ...
            // Phase 1: Trip mutations do NOT drive operational current odometer
            // Operational current is derived from qualifying expense evidence only
        }
     ...
     * Phase 2: Removed cross-domain validation against Vehicle.currentOdometer
     * to allow backdated trip entry. Historical trips may have start odometer
     * lower than today's operational current odometer.
     */
    private void validateOdometerReadings(Integer startOdometer, Integer endOdometer, Vehicle vehicle) {
        ...
        // Phase 2: Removed validation that rejected startOdometer < vehicle.getCurrentOdometer()
        // This allows historical/backdated trip entry
    }
