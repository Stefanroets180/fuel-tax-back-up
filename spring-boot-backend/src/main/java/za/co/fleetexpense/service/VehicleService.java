...
public class VehicleService {
    ...
    public VehicleDTO create(VehicleCreateRequest request, UUID organizationId, UUID userId, UserRole creatorRole) {
        ...
        // Phase 1: Capture operational baseline if provided
        // If operationalBaselineOdometer is supplied, set both baseline and current to that value
        // If not supplied, baseline remains NULL (BASELINE_UNKNOWN state)
        // Legacy currentOdometer field is treated as untrusted continuity only, not converted to baseline
        if (request.getOperationalBaselineOdometer() != null) {
            vehicle.setOperationalBaselineOdometer(request.getOperationalBaselineOdometer());
            vehicle.setCurrentOdometer(request.getOperationalBaselineOdometer());
        } else {
            vehicle.setOperationalBaselineOdometer(null);
            // Legacy currentOdometer is untrusted continuity - preserve for backward compatibility
            // but do NOT convert it to operational baseline
    ...
    /**
     * Canonical operational odometer recalculation authority.
     * Calculates persisted derived operational current mileage from:
     * 1. Known operational baseline (operationalBaselineOdometer)
     * 2. Active qualifying operational expense evidence (FUEL_LOG, TIRES, qualifying MECHANIC_SERVICE)
     *
     * Formula:
     * - IF baseline != NULL AND qualifyingMax != NULL: current = MAX(baseline, qualifyingMax)
     * - ELSE IF baseline != NULL: current = baseline
     * - ELSE IF qualifyingMax != NULL: current = qualifyingMax
     * - ELSE: preserve existing stored currentOdometer (BASELINE_UNKNOWN state)
     *
     * This is the ONLY method that should decide and persist Vehicle.currentOdometer.
     * Other services must REQUEST recalculation via this method.
     *
     * Phase 1: Uses pessimistic write lock to prevent concurrent modifications during recalculation.
     */
    @Transactional
    public VehicleDTO recalculateOperationalOdometer(UUID vehicleId) {
        Vehicle vehicle = vehicleRepository.findByIdWithLock(vehicleId)
                .orElseThrow(() -> new ResourceNotFoundException("Vehicle not found"));

        Vehicle saved = recalculateLockedOperationalOdometer(vehicle);
    ...
    /**
     * Multi-vehicle operational odometer recalculation with deterministic locking.
     * Prevents deadlock by acquiring locks in sorted order.
     *
     * Phase 1: Deduplicates, sorts, and locks all affected vehicle rows in deterministic order.
     */
    @Transactional
    public void recalculateOperationalOdometers(java.util.Collection<UUID> vehicleIds) {
        if (vehicleIds == null || vehicleIds.isEmpty()) {
            return;
        }

        // Deduplicate and sort deterministically
        java.util.List<UUID> sortedIds = vehicleIds.stream()
                .distinct()
                .sorted()
                .toList();

        log.info("Recalculating operational odometer for {} vehicles in sorted order: {}", 
                sortedIds.size(), sortedIds);

        // Acquire locks and recalculate in sorted order
        for (UUID vehicleId : sortedIds) {
            try {
                Vehicle vehicle = vehicleRepository.findByIdWithLock(vehicleId)
                        .orElseThrow(() -> new ResourceNotFoundException("Vehicle not found"));
                recalculateLockedOperationalOdometer(vehicle);
            } catch (Exception e) {
                log.error("Failed to recalculate operational odometer for vehicle {}", vehicleId, e);
    ...
    /**
     * Internal canonical operational odometer calculation and persistence.
     * This is the ONLY implementation of the operational formula and persistence.
     * Vehicle must already be locked by caller (pessimistic lock).
     *
     * Formula:
     * - IF baseline != NULL AND qualifyingMax != NULL: current = MAX(baseline, qualifyingMax)
     * - ELSE IF baseline != NULL: current = baseline
     ...
     * - ELSE: preserve existing stored currentOdometer (BASELINE_UNKNOWN state)
     */
    private Vehicle recalculateLockedOperationalOdometer(Vehicle vehicle) {
        ...
        if (baseline != null && qualifyingMax != null) {
            newCurrent = Math.max(baseline, qualifyingMax);
        } else if (baseline != null) {
            newCurrent = baseline;
        } else if (qualifyingMax != null) {
            newCurrent = qualifyingMax;
        } else {
            // BASELINE_UNKNOWN: preserve existing stored value for continuity
            newCurrent = vehicle.getCurrentOdometerStored();
        }

        vehicle.setCurrentOdometer(newCurrent);
        Vehicle saved = vehicleRepository.save(vehicle);

        // Refresh downstream operational consumers
        odometerDriftAlertService.checkAndCreateAlert(vehicleId);
        mechanicServiceAlertService.checkVehicleAlerts(vehicleId);

        // Phase 1: Tyre rotation status is calculated dynamically on read using vehicle.getCurrentOdometer()
        // No persisted refresh needed - downward odometer changes are reflected on next read.
        // TyreRotationService.toWarningDTO() uses vehicle.getCurrentOdometer() for kmRemaining calculations

        log.info("Recalculated operational odometer for vehicle {} - baseline: {}, qualifyingMax: {}, newCurrent: {} km",
                vehicleId, baseline, qualifyingMax, newCurrent);
        return saved;
    }

    /**
     * Explicit operational baseline correction.
     * Updates the trusted operational starting point and recalculates derived current mileage.
     * Requires justification for audit trail.
     * Authorization: Organization owner, SUPER_ADMIN, ADMIN, or MANAGER with appropriate permissions.
     * Assigned fleet drivers are NOT allowed to correct the operational baseline.
     */
    @Transactional
    public VehicleDTO correctOperationalBaseline(UUID vehicleId, Integer newBaseline, String justification, UUID userId) {
        Vehicle vehicle = vehicleRepository.findById(vehicleId)
                .orElseThrow(() -> new ResourceNotFoundException("Vehicle not found"));

        // Validation
        if (newBaseline == null) {
            throw new ValidationException("New baseline cannot be null");
        }
        if (newBaseline < 0) {
            throw new ValidationException("New baseline cannot be negative");
        }
        if (justification == null || justification.trim().isEmpty()) {
            throw new ValidationException("Justification is required for baseline correction");
        }

        // Authorization check using PermissionService (organization-scoped)
        UUID organizationId = vehicle.getOrganization().getId();
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new ResourceNotFoundException("User not found"));
        
        // Check if user is organization owner (null-safe)
        boolean isOrgOwner = java.util.Objects.equals(vehicle.getOrganization().getOwnerId(), userId);
        
        // Check CORRECT_BASELINE permission for this organization (MANAGER/ADMIN)
        // SUPER_ADMIN bypasses via PermissionService.isAllowed()
        boolean hasPermission = permissionService.isAllowed(organizationId, "ODOMETER_MANAGEMENT", "CORRECT_BASELINE", user.getRole().name());
        
        if (!isOrgOwner && !hasPermission) {
            throw new ValidationException("You do not have permission to correct the operational baseline for this vehicle");
        }

        vehicle.setOperationalBaselineOdometer(newBaseline);
        Vehicle saved = vehicleRepository.save(vehicle);
        log.info("Corrected operational baseline for vehicle {} to {} km by user {}. Justification: {}",
                vehicleId, newBaseline, userId, justification);

        // Recalculate derived current from new baseline (canonical authority)
        return recalculateOperationalOdometer(vehicleId);
    }
